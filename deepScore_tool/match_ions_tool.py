...
class MATCH(object):
    def __init__(self, workpath, file_name, ppm_threshold):
        self.internal_ions_maxlength = 2
        self.workpath = workpath
        self.file_name = file_name
        self.ppm_threshold = float(ppm_threshold)
    ...
    def get_by_mz(self, sequence, charge):
        ...
        y_mass_list = np.divide(np.cumsum(np.array(peptide_list_mass[::-1])) + self.PROTON * charge + self.H2O, charge)[
                      :-1]
    ...
    def match_ions(self):
        ...
        for i in tqdm(range(len(mgf))):
            ...
            for index in range(len(bs_ions1)):  ##regular ions
                ...
                matched_mz = []
                intensity = []
                ion_type = ['b' + str(index + 1) + '+', 'b' + str(index + 1) + '++',
                            'y' + str(len(bs_ions1) - index) + '+', 'y' + str(len(bs_ions1) - index) + '++']
                dif_a1 = ((mz - as_ions1[index]) / as_ions1[index]) * 10 ** 6
                try:
                    if np.min(abs(dif_a1)) < self.ppm_threshold:
                        matched_index = np.where(
                            np_mz_intensity == np.max(np_mz_intensity[np.where(abs(dif_a1) < self.ppm_threshold)]))[0]
                ...
                try:
                    if np.min(abs(dif_a2)) < self.ppm_threshold:
                        matched_index = np.where(
                            np_mz_intensity == np.max(np_mz_intensity[np.where(abs(dif_a2) < self.ppm_threshold)]))[0]
                        np_mz_intensity = np.delete(np_mz_intensity, matched_index, 0)
                        mz = np_mz_intensity[:, 0]
                except:
                    pass
                ...
                try:
                    if np.min(abs(dif_b1)) < self.ppm_threshold:
                        matched_index = np.where(
                            np_mz_intensity == np.max(np_mz_intensity[np.where(abs(dif_b1) < self.ppm_threshold)]))[0]
                        matched_mz.append(str(mz[matched_index][0]))
                        intensity.append(str(np_mz_intensity[matched_index, 1][0]))
                        ions_error.append([str(mz[matched_index][0]), str(dif_b1[matched_index][0]),
                                           str((np_mz_intensity[:, 1][matched_index][0]) / max_intensity)])
                        np_mz_intensity = np.delete(np_mz_intensity, matched_index, 0)
                        mz = np_mz_intensity[:, 0]
                    else:
                ...
                try:
                    if np.min(abs(dif_b2)) < self.ppm_threshold:
                        matched_index = np.where(
                            np_mz_intensity == np.max(np_mz_intensity[np.where(abs(dif_b2) < self.ppm_threshold)]))[0]
                        matched_mz.append(str(mz[matched_index][0]))
                        intensity.append(str(np_mz_intensity[matched_index, 1][0]))
                        ions_error.append([str(mz[matched_index][0]), str(dif_b2[matched_index][0]),
                                           str((np_mz_intensity[:, 1][matched_index][0]) / max_intensity)])
                        np_mz_intensity = np.delete(np_mz_intensity, matched_index, 0)
                        mz = np_mz_intensity[:, 0]
                    else:
                ...
                try:
                    if np.min(abs(dif_y1)) < self.ppm_threshold:
                        matched_index = np.where(
                            np_mz_intensity == np.max(np_mz_intensity[np.where(abs(dif_y1) < self.ppm_threshold)]))[0]
                        matched_mz.append(str(mz[matched_index][0]))
                        intensity.append(str(np_mz_intensity[matched_index, 1][0]))
                        ions_error.append([str(mz[matched_index][0]), str(dif_y1[matched_index][0]),
                                           str((np_mz_intensity[:, 1][matched_index][0]) / max_intensity)])
                        np_mz_intensity = np.delete(np_mz_intensity, matched_index, 0)
                        mz = np_mz_intensity[:, 0]
                    else:
                ...
                try:
                    if np.min(abs(dif_y2)) < self.ppm_threshold:
                        matched_index = np.where(
                            np_mz_intensity == np.max(np_mz_intensity[np.where(abs(dif_y2) < self.ppm_threshold)]))[0]
                        matched_mz.append(str(mz[matched_index][0]))
                        intensity.append(str(np_mz_intensity[matched_index, 1][0]))
                        ions_error.append([str(mz[matched_index][0]), str(dif_y2[matched_index][0]),
                                           str((np_mz_intensity[:, 1][matched_index][0]) / max_intensity)])
                        np_mz_intensity = np.delete(np_mz_intensity, matched_index, 0)
                        mz = np_mz_intensity[:, 0]
                    else:
