...
class BiLSTM_CRF(nn.Module):
    def __init__(self, hidden_dim, features_number, layer_num=2, batch_size=16, label_number=4):
        ...
        self.lstm = nn.LSTM(self.features_number, hidden_dim // 2, num_layers=self.layer_num, bidirectional=True,
                            dropout=0.2).cuda()
    ...
    def get_lstm_features(self, sentence, length):
        ...
        packed = nn_utils.rnn.pack_padded_sequence(torch.index_select(sentence, 0, sorted_index), sorted_length,
                                                   batch_first=True)
        ...
        unpacked = torch.index_select(unpacked, 0,
                                      unsorted_index)  # Shape : batch, seq_len, hidden_size * num_directions
        lstm_out = self.out_layer(unpacked)
        lstm_out = lstm_out.reshape(lstm_out.shape[0], lstm_out.shape[1], self.label_number,
                                    -1)  # Shape : batch,seq_len,label_number,11
        return lstm_out

    def forward(self, x, y_true, batch_length):
        x = x.permute(0, 2, 1)
        print(x.shape)
        out = self.get_lstm_features(x.reshape(len(batch_length), max(batch_length), self.features_number),
                                     batch_length)
        mask = y_true != -1
        loss = self.crf(out.view(out.size(0), -1, 11), y_true.view(y_true.size(0), -1),
