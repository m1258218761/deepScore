...
class Pscore_Model(object):
    def __init__(self,NCE='30',run_model='Train',pretrained=False,BATCH_SIZE=16,Label_number=4,features_size=105,weights=[]):
        self.NCE = NCE
    ...
    def Train(self):
        ...
        Data = data('./Data',self.Label_number,discretization=0,train_file=train_file,validation_file=validation_file)
            ...
            for b_index,b_data in enumerate(Train_data):
                ...
                if ep_count % ep_validation == 0:
                    with torch.no_grad():
                        ...
                        print('+-1 rate : ' + str(t))
                        self.save_model(model=model,Y_T=Y_T,Y_P=Y_P,cp=cp,p_report=p_report,t=t,tacc=tacc,bestacc=bestacc,test_loss=test_loss,bestloss=bestloss)
            with open('./Log/model_2.txt','a+') as r:
